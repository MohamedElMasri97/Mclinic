...
export default {
    ...
    created(){
        ...
        if(!(self.$route.params.req_id && self.$route.params.offerid && self.$route.params.providerid) ){
            self.$router.push({name:'pharmacyreqs'});
        }else{
            self.refresh();
        }
    ...
    methods:{
        back(){
            var self = this;
            this.$router.push({name:'pharmacyoffers',params:{req_id:self.$route.params.req_id}})
        },
        refresh(){
            var self = this;
            self.offers = [];
            ...
            $.ajax({
                ...
                action:'fetch_patient_detailed_pharmacy_offer',
        ...
        accept(){
            ...
            $.ajax({
                ...
                console.log('successfull query and results is ' + results);
                if(results[0]){
                    self.$router.push({name:'pharmacyreqs',params:{alert:{state:'success',msg:'The offer has been accepted and moved to appontments.'}}});
                }else{
                    self.errors =  ['بيانات هذا الطلب غير صحيحة'];
                    self.show_error =true;
                }
