<template id='temp'>
  <div id="root">
    ...
    <div v-if="show" class="header">
      ...
      <hr  class='hr' />
      <h4>تفاصيل العرض</h4>        
      <p>العرض المقدم من قبل الطبيب: {{offer.offernote}}</p>
      <hr  class='hr' />
      <p>اسم الطبيب: {{provider.usernmae}}</p>
      <hr  class='hr' />
      <p>عنوان الطبيب: {{provider.np}}, {{$store.state.citymap[provider.city]}}</p>
      <hr  class='hr' />
...
export default {
    ...
    created(){
        ...
        if(!(self.$route.params.req_id && self.$route.params.offerid && self.$route.params.providerid) ){
            self.$router.push({name:'doctorreqs'});
        }else{
            self.refresh();
        }
    ...
    methods:{
        back(){
            var self = this;
            this.$router.push({name:'doctoroffers',params:{req_id:self.$route.params.req_id}})
        },
        refresh(){
            var self = this;
            self.offers = [];
            ...
            $.ajax({
                ...
                action:'fetch_patient_detailed_doctor_offer',
        ...
        accept(){
            ...
            $.ajax({
                ...
                console.log('successfull query and results is ' + results);
                if(results[0]){
                    self.$router.push({name:'doctorreqs',params:{alert:{state:'success',msg:'The offer has been accepted and moved to appontments.'}}});
                }else{
                    self.errors =  ['بيانات هذا الطلب غير صحيحة'];
                    self.show_error =true;
                }
