...
export default {
  ...
  methods: {
    ...
    refresh(){
      ...
      $.ajax({
        ...
        data:{
          ...
          action:'fetch_patient_reqs_for_lab',
        ...
        try{
          ...
          if(results[0]){
            ...
            self.errors.push('You have no lab requests.');
    ...
    details(element){
      this.$router.push({name:'laboffers',params:{req_id:element.req_id}})
    }
  },
  created() {
    var self = this;
    self.refresh();
    console.log("you are in the labreqs vue comp");
  }
};
</script>
<style scoped>
