<template>
  <div id="root">
    ...
    <b-form @submit="onSubmit" v-if="show" class="header">
      <b-form-group
        id="input-group-1"
        label="ادرج التحاليل التي تود اجرائها"
        label-for="input-1"
      >
        <b-form-textarea
          id="textarea"
          v-model="form.note"
          placeholder="التحاليل المطلوبة..."
          rows="5"
          max-rows="6"
          required
        ></b-form-textarea>
      ...
      <b-form-group id="input-group-6" label="لماذا تريد طلب تحليل" label-for="input-6">
        <b-form-input
          id="input-6"
          v-model="form.why"
          type="text"
          required
          placeholder="مثلا... وفقا لطلب طبيب"
        ></b-form-input>
      </b-form-group>
      ...
      <b-form-group id="input-group-5" label="اقرب نقطة دالة لموقعك الحالي" label-for="input-5">
        <b-form-input
          id="input-5"
          v-model="form.np"
          type="text"
...
export default {
  ...
  created(){
    var self = this;
    c.checkreq("3").then(function(state) {
      if (state == 2) {
        self.$router.push({
          name: "requesthome",
          params: {
            alert: {
              msg: "sorry, you already have a doctor request.",
              state: "warning"
            }
          }
        });
        ...
        $.ajax({
          ...
          .done(function(data) {
            try{
              ...
              console.log("done response at ask lab" + results);
              if (results[0]) {
                self.$router.push({
                  name: "requesthome",
                  params: {
              ...
              console.log("fail response at ask lab, error: " + err + '. data: ' + data);
              self.state = "warning";
              self.show_error = true;
              self.errors = ["عفوا حدث خطأ حاول مرة اخرى لاحقا"];
            }
