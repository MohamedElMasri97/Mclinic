        ...
        <b-row v-for="element in appointments" :key="element.req.req_id" style="width:100%;margin:10px;">
          <b-card class="cardcolor">
            ...
            <template v-slot:footer>
              ...
              <hr  class='hr' />
              <p>lab {{element.provider.title}} في {{element.provider.np}}, {{$store.state.citymap[element.provider.city]}}.</p>
...
export default {
  ...
  methods: {
    ...
    details(element){
      this.$router.push({name:'patientdetailedlabappointment',params:{req_id:element.req.req_id,offerid:element.offer.offerid,providerid:element.provider.fuserid}})
    }
  },
  created() {
    var self = this;
    self.refresh();
    console.log("you are in the labappointments vue comp");
  }
};
</script>
<style scoped>
