<template>
  <div id="root">
    ...
    <hr />
    <p>اسم المستخدم:  {{$store.state.doctor_offer_element.name}}</p>
    <hr />
    <p>جنس المستخدم:   {{$store.state.gendermap[$store.state.doctor_offer_element.gender]}}</p>
    <hr />
    <p>تاريخ ميلاد المستخدم:   {{$store.state.doctor_offer_element.birth}}</p>
    <hr />
    <p>المدينة:   {{$store.state.citymap[$store.state.doctor_offer_element.city]}}</p>
    <hr />
    <p>اقرب نقطة دالة:   {{$store.state.doctor_offer_element.np}}</p>
    <hr />
    <p>الهاتف:  {{$store.state.doctor_offer_element.phone}}</p>
    <hr />
    <p>وقت الطلب: {{$store.state.doctor_offer_element.ts}}</p>
    <hr />
    <p>{{$store.state.doctor_offer_element.note}}</p>
    <hr />
...
export default {
  created(){
    var self = this;
      console.log(self.$store.state.doctor_offer_element);

    if(!self.$store.state.doctor_offer_element){
      self.$router.push({name:'doctor_trending'})
    }
  },
  data() {
    return {
  ...
  methods: {
    onCancel() {
      this.$router.push({ name: "doctor_trending" });
    },
    onSubmit(evt) {
      evt.preventDefault();
      c.checkme();
      ...
      console.log("submitting");
        $.ajax({
          ...
          data: {
            action: "rig_doctor_offer",
            pass: self.$store.state.phrase,
            patientid: self.$store.state.doctor_offer_element.id,
            reqid: self.$store.state.doctor_offer_element.req,
