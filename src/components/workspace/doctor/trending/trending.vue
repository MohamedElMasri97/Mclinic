    ...
    <b-card-group deck>
      <b-row v-for="element in elements" :key="element.req_id" class="mrg">
        <b-card class="cardcolor">
          <b-card-text>{{element.note}}</b-card-text>
      ...
      <!-- <b-row  class="mrg">
        <b-card class="cardcolor" >
          <b-card-text>dfgsdgsdfg</b-card-text>
          <template v-slot:footer>
            <p>اسم صاحب الطلب .</p>
            <p>Posted at: fgsdfg</p>
            <hr />
            <b-button variant="primary" @click="offer(element)">ارسال عرض</b-button>
          </template>
        </b-card>
...
export default {
  ...
  created() {
    ...
    $.ajax({
      ...
      data: {
        ...
        action: "fetch_patients_fordoctor",
        city: "all",
        limit: self.limit
      }
    })
  ...
  methods: {
    ...
    offer(element) {
      console.log(element);
      this.$store.state.doctor_offer_element = element;
      this.$router.push({ name: "doctor_offer" });
    }
  }
};
</script>
