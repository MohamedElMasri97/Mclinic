<template>
   <div id="root">
    ...
    <b-card-group deck v-if="showcard">
      <b-row v-for="element in elements" :key="element.req.req_id" class="mrg">
        <b-card class="cardcolor">
          <b-card-text>
            <p class="inline"><span>اسم المستخدم</span> <span> {{element.patient.username}}</span> 
               <span> متواجد في </span>  <span> {{element.req.np}},</span>  <span> {{$store.state.citymap[element.req.city]}},</span>
      ...
      <b-row class="mrg">
        <b-card>
          <b-card-text>
            ...
            <hr />
            <p >سبب الطلب:  {{card.req.why}}</p>
            <hr />
            <p v-if="card.req.barcode != ''">الباركود:  {{card.req.barcode}}</p>
            <hr />
            ...
            <hr />
            <b-button variant="warning" @click="del(card);fetch();delstatement();">حذف الموعد</b-button>
  ...
  methods: {
    fetch() {
      ...
      $.ajax({
        ...
        data: {
          ...
          action:'fetch_pharmacy_appointments',
    ...
    del(element){
      ...
      $.ajax({
        ...
        data: {
          pass: self.$store.state.phrase,
          action:'del_pharmacy_appointment',
        ...
        .done(function(data) {
          ...
          try {
            ...
            if (results[0]) {
              ...
              console.log("error while fetching patients ,data: " + data);
              self.errors = ["ليس لديك اي مواعيد"];
              self.state = "warning";
              self.show_error = true;
            }
          } catch (err) {
